// app/dashboard/student/pre-test/_components/PreTestClient.tsx
"use client";

import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import { cn } from "@/lib/utils";
import type { PreTestData, PreTestResult, TestQuestion } from "../page";
...
function AnswerInput({
  ...
  const base = "w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#0F1829] border-2 border-[#E2E8F0] dark:border-[#1E3A5F] rounded-2xl text-sm focus:outline-none focus:border-[#3B82C4] dark:focus:border-[#3B82C4] text-[#1E293B] dark:text-[#E2E8F0] placeholder:text-[#94A3B8] transition-colors font-nunito font-bold";
          ...
          return (
            <button
              ...
              className={cn(
                ...
                selected
                  ? "border-[#3B82C4] bg-[#EFF6FF] dark:bg-[#0F1829] text-[#1E293B] dark:text-[#E2E8F0]"
                  : "border-[#E2E8F0] dark:border-[#1E3A5F] bg-white dark:bg-[#0A1120] text-[#1E293B] dark:text-[#E2E8F0] hover:border-[#BFDBFE] dark:hover:border-[#1E3A5F]"
              )}
            >
              <span className={cn(
                "w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-extrabold shrink-0 transition-all",
                selected ? "bg-[#3B82C4] text-white" : "bg-[#EEF2F8] dark:bg-[#162032] text-[#64748B] dark:text-[#94A3B8]"
              )}>
                {letter}
              </span>
...
function QuestionCard({
  ...
  return (
    <div className="fade-in flex flex-col gap-5">
      {/* Q number + context + difficulty */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-full bg-[#3B82C4] text-white flex items-center justify-center text-sm font-extrabold font-nunito">
            {q.questionNum}
          </div>
          <div>
            <p className="text-[11px] font-bold text-[#3B82C4] uppercase tracking-wider">
      ...
      {/* Question text */}
      <div className="px-5 py-4 bg-[#EFF6FF] dark:bg-[#0F1829] border border-[#BFDBFE] dark:border-[#1E3A5F] rounded-2xl">
      ...
      {/* Answer */}
      <div>
        <label className="block text-[11px] font-extrabold tracking-widest uppercase text-[#3B82C4] mb-2">
          Your Answer
        </label>
    ...
    <div className="flex flex-col gap-5 fade-in">
        ...
        {questions.map((q) => {
          ...
          return (
            <div key={q.id} className="flex items-start gap-3 px-4 py-3 bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] rounded-xl">
              <div className="w-7 h-7 rounded-full bg-[#3B82C4] text-white flex items-center justify-center text-[11px] font-extrabold shrink-0 mt-0.5">
                {q.questionNum}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[12px] font-semibold truncate text-[#1E293B] dark:text-[#E2E8F0]">{q.questionText}</p>
                <p className={cn("text-[12px] font-bold mt-0.5", ans ? "text-[#3B82C4]" : "text-[#94A3B8] italic")}>
                  {ans
              ...
              <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0", ans ? "bg-[#DBEAFE] text-[#3B82C4]" : "bg-[#EEF2F8] dark:bg-[#162032] text-[#94A3B8]")}>
                {ans ? "Answered" : "Blank"}
              </span>
            </div>
          );
      ...
      <div className="flex gap-3 pt-2">
        <button
          onClick={onBack}
          disabled={submitting}
          className="flex-1 py-3 rounded-2xl font-nunito font-bold text-sm border-2 border-[#E2E8F0] dark:border-[#334155] text-[#64748B] hover:border-[#3B82C4] hover:text-[#3B82C4] transition-all disabled:opacity-40"
        >
          ← Go Back
        </button>
        <button
          onClick={onSubmit}
          disabled={submitting}
          className="flex-[2] py-3 rounded-2xl font-nunito font-extrabold text-sm bg-[#3B82C4] hover:bg-[#2563EB] text-white shadow-[0_4px_14px_rgba(59,130,196,0.35)] transition-all disabled:opacity-50"
        >
...
}: { result: PreTestResult; studentName: string; onDone: () => void }) {
  const pct = result.percentScore;
  const color = pct >= 75 ? "#4A9B7F" : pct >= 50 ? "#F59E0B" : "#E05C5C";
  const label = pct >= 75 ? "Great start!" : pct >= 50 ? "Good effort!" : "Keep practicing!";
  ...
  return (
    <div className="flex flex-col items-center text-center gap-6 py-4 fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-[#64748B] dark:text-[#94A3B8]">
          Pre-test completed, {studentName.split(" ")[0]}. Your teacher will see this result.
        </p>
        <p className="text-[11px] text-[#94A3B8] mt-1">Taken on {result.submittedAt}</p>
      </div>

      <div className="w-full px-5 py-4 bg-[#EFF6FF] dark:bg-[#0F1829] border border-[#BFDBFE] dark:border-[#1E3A5F] rounded-2xl text-left">
        <p className="text-xs font-bold text-[#3B82C4] mb-2 uppercase tracking-wider">What happens next?</p>
        <ul className="text-sm text-[#1E293B] dark:text-[#E2E8F0] flex flex-col gap-1.5">
          <li className="flex items-start gap-2"><span className="mt-0.5">📚</span> Work through the assigned modules in your dashboard</li>
          <li className="flex items-start gap-2"><span className="mt-0.5">📊</span> Your teacher will compare your pre-test score with the post-test</li>
          <li className="flex items-start gap-2"><span className="mt-0.5">✅</span> Take the post-test after completing the module</li>
        </ul>
      </div>

      <button
        onClick={onDone}
        className="w-full py-3 rounded-2xl font-nunito font-extrabold text-sm bg-[#3B82C4] hover:bg-[#2563EB] text-white shadow-[0_4px_14px_rgba(59,130,196,0.35)] transition-all"
      >
        Back to Dashboard →
      </button>
    </div>
...
function AlreadyTakenScreen({
  ...
  return (
    <div className="flex flex-col items-center text-center gap-6 py-4 fade-in">
      ...
      <div>
        <p className="font-nunito text-2xl font-extrabold mb-1">Pre-test Already Completed</p>
        <p className="text-sm text-[#64748B] dark:text-[#94A3B8]">
          You already took this pre-test on {result.submittedAt}, {studentName.split(" ")[0]}.
        </p>
      </div>
...
function NoTestScreen({ onDone }: { onDone: () => void }) {
  return (
    <div className="flex flex-col items-center text-center gap-5 py-8 fade-in">
      ...
      <div>
        <p className="font-nunito text-xl font-extrabold mb-1">No Pre-test Available</p>
        <p className="text-sm text-[#64748B] dark:text-[#94A3B8] max-w-xs mx-auto">
          Your teacher hasn&apos;t set up a pre-test yet. Check back later or ask your teacher.
        </p>
      </div>
      <button
        onClick={onDone}
        className="px-6 py-2.5 rounded-xl font-nunito font-bold text-sm border-2 border-[#E2E8F0] dark:border-[#334155] text-[#64748B] hover:border-[#3B82C4] hover:text-[#3B82C4] transition-all"
      >
        Back to Dashboard
      </button>
    </div>
...
export default function PreTestClient({
  ...
  alreadyTaken: PreTestResult | null;
  studentName:  string;
}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResult(json.result as PreTestResult);
      setPhase("done");
    } catch {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex-1 mr-4">
          ...
          <div className="h-2 bg-[#EEF2F8] dark:bg-[#162032] rounded-full overflow-hidden">
            <div
              className="h-full bg-[#3B82C4] rounded-full transition-all duration-500"
        ...
        <div className={cn(
          ...
          data.timeLimit !== null
            ? (remainingFmt && parseInt(remainingFmt) < 5 * 60
                ...
                : "bg-[#EFF6FF] dark:bg-[#0F1829] text-[#3B82C4]")
        ...
        <>
          ...
          <div className="flex gap-3 mt-6">
            <button
              ...
              className="flex-1 py-3 rounded-2xl font-nunito font-bold text-sm border-2 border-[#E2E8F0] dark:border-[#334155] text-[#64748B] hover:border-[#3B82C4] hover:text-[#3B82C4] transition-all disabled:opacity-30"
            >
              ← Previous
            </button>
            ...
            {!isLast ? (
              <button
                ...
                className="flex-[2] py-3 rounded-2xl font-nunito font-extrabold text-sm bg-[#3B82C4] hover:bg-[#2563EB] text-white shadow-[0_4px_14px_rgba(59,130,196,0.25)] transition-all"
              >
                Next →
              </button>
            ) : (
              <button
                ...
                className="flex-[2] py-3 rounded-2xl font-nunito font-extrabold text-sm bg-[#3B82C4] hover:bg-[#2563EB] text-white shadow-[0_4px_14px_rgba(59,130,196,0.35)] transition-all"
              >
                Review Answers →
              </button>
            )}
...
function Shell({ children, studentName }: { children: React.ReactNode; studentName: string }) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F0F6FF] dark:bg-[#0A1120] text-[#1E293B] dark:text-[#E2E8F0]">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-white dark:bg-[#0F1829] border-b border-[#BFDBFE] dark:border-[#1E3A5F] px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push("/dashboard/student")}
              className="text-sm font-bold text-[#64748B] hover:text-[#3B82C4] transition-colors"
            >
              ← Dashboard
            </button>
            <span className="text-[#BFDBFE] dark:text-[#1E3A5F]">/</span>
            <div className="flex items-center gap-2">
              <span className="text-lg">📋</span>
              <span className="font-nunito font-extrabold text-[15px] text-[#3B82C4]">Pre-test</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#DBEAFE] dark:bg-[#1e3a5f] flex items-center justify-center text-[#3B82C4] text-[11px] font-extrabold font-nunito">
