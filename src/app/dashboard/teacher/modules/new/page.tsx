...
interface SimpleStage {
  ...
  // ranking
  rankItems:     RankItem[];
  correctOrder:  number[]; // indices of rankItems in correct order
  // open_ended
  answerKey:    string;
...
const PHASE_OF = (n: number) => {
  if (n <= 3)  return { label: "Understanding", color: "#3B82C4", ring: "ring-blue-500"  };
  if (n <= 7)  return { label: "Analysis",      color: "#8B5CF6", ring: "ring-purple-500" };
  if (n <= 10) return { label: "Solution",      color: "#F59E0B", ring: "ring-amber-500"  };
  return               { label: "Reflection",   color: "#4A9B7F", ring: "ring-emerald-500" };
};
...
function TextInput({
  ...
  const cls =
    "w-full rounded-lg border border-white/10 bg-slate-800/60 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none";
  return multiline ? (
    <textarea
...
function RankingEditor({
  ...
  // Keep ids in sync when items are added/removed
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          type="number"
          min={0}
          className="w-24 rounded-lg border border-white/10 bg-slate-800/60 px-3 py-1.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <div className="space-y-2">
        ...
        {chips.length < 6 && (
          <button
            type="button"
            onClick={addChip}
            className="w-full rounded-lg border border-dashed border-slate-600 py-2 text-xs text-slate-400 hover:border-blue-500 hover:text-blue-400 transition-colors"
          >
      ...
      <div>
        ...
        <textarea
          className="w-full rounded-lg border border-white/10 bg-slate-800/60 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none min-h-[90px]"
...
function StageEditor({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Mode-specific options + answer key */}
...
export default function ModuleNewPage() {
  ...
  async function handleSubmit(saveAsDraft: boolean) {
    ...
    try {
      ...
      router.push("/dashboard/teacher/modules");
    } catch {
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex gap-2">
            {["Module Info", "Module Quiz", "12 Stages", "Review"].map((lbl, i) => (
              <div key={i} className="flex-1 h-1.5 rounded-full" style={{ background: i === 0 ? "#3B82C4" : "#1e293b" }} />
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-6 space-y-5">
            ...
            <div className="space-y-3">
              ...
              {showNewCtx && (
                <div className="rounded-xl border border-blue-500/30 bg-blue-950/20 p-4 space-y-3">
                  <p className="text-xs font-semibold text-blue-400 uppercase tracking-wide">Create New Context</p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-shrink-0">
                      ...
                      <input
                        className="w-14 rounded-lg border border-white/10 bg-slate-800/60 px-2 py-2 text-xl text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
                        value={newCtx.icon}
                        maxLength={4}
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        className="w-full rounded-lg border border-white/10 bg-slate-800/60 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        value={newCtx.label}
                  ...
                  <button
                    ...
                    className="w-full rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed py-2 text-sm font-semibold text-white transition-colors"
                  >
            ...
            <div>
              ...
              <input
                type="number"
                min={10}
                className="w-32 rounded-lg border border-white/10 bg-slate-800/60 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <button
            ...
            className="w-full rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed py-3 font-semibold transition-colors"
          >
            Next: Module Quiz →
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-[#0D1117] text-white p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex gap-2">
            {["Module Info", "Module Quiz", "12 Stages", "Review"].map((lbl, i) => (
              <div key={i} className="flex-1 h-1.5 rounded-full" style={{ background: i <= 1 ? "#3B82C4" : "#1e293b" }} />
            ))}
          </div>

          <div className="rounded-2xl border border-blue-500/20 bg-blue-950/10 p-4 text-sm text-blue-300 space-y-1">
            <p className="font-semibold">📝 About the Module Quiz</p>
            <p className="text-slate-400">These questions will appear as a separate quiz that students must answer <strong className="text-white">after completing all 12 stages</strong>. Use any of the 3 question types. You can skip this step to create the module without a quiz.</p>
          </div>

          <ModuleQuizEditor questions={quizQuestions} onChange={setQuizQuestions} dark />

          <div className="flex gap-3">
            <button type="button" onClick={() => { setQuizQuestions([]); setStep("stages"); }}
            ...
            <button type="button" onClick={() => setStep("stages")}
              disabled={quizQuestions.some(q => !q.questionText.trim()) && quizQuestions.length > 0}
              className="flex-1 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed py-3 text-sm font-semibold transition-colors">
        ...
        <aside className="w-56 flex-shrink-0 border-r border-white/10 flex flex-col">
          ...
          <div className="p-3 border-t border-white/10 space-y-2">
            ...
            <button
              ...
              className="w-full rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed py-2 text-sm font-semibold transition-colors"
            >
              Review →
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#0D1117] text-white p-6">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex gap-2">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="flex-1 h-1.5 rounded-full bg-blue-600" />
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-900/60 overflow-hidden">
          <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
            ...
            <button type="button" onClick={() => setStep("quiz")} className="text-xs text-blue-400 hover:text-blue-300">Edit</button>
          </div>
          {quizQuestions.length === 0 ? (
            <p className="px-4 py-3 text-xs text-slate-500">No quiz — students skip directly to the completion screen after Stage 12.</p>
          ) : (
            <div className="divide-y divide-white/5">
              {quizQuestions.map((q) => (
                <div key={q.questionNum} className="px-4 py-2.5 flex items-center gap-3 text-sm">
                  <span className="w-5 h-5 rounded-full bg-blue-600/30 text-blue-300 text-[10px] font-black flex items-center justify-center shrink-0">{q.questionNum}</span>
        ...
        <div className="rounded-xl border border-white/10 bg-slate-800/40 px-4 py-3 text-xs text-slate-400 space-y-1">
          <p><span className="font-semibold text-slate-300">Save as Draft</span> — module is saved privately. Not visible to any section. You can continue editing and publish later.</p>
          <p><span className="font-semibold text-slate-300">Publish Module</span> — module becomes active and is assigned to your sections. All 12 stages must be complete.</p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 rounded-xl bg-blue-600 hover:bg-blue-500 py-3 text-sm font-semibold text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
