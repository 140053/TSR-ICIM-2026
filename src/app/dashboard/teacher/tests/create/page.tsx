...
interface QuestionDraft {
  ...
  answer:       string;          // for MC: "A"|"B"|"C"|"D"; for open: the value
  choices:      [string, string, string, string]; // only used when answerType === "multiple_choice"
...
function MCEditor({ q, upd }: { q: QuestionDraft; upd: (q: QuestionDraft) => void }) {
  ...
  const setCorrect = (letter: ChoiceLetter) => upd({ ...q, answer: letter });
        ...
        return (
          <div key={letter} className="flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                "w-8 h-8 rounded-full border-2 flex items-center justify-center text-[12px] font-extrabold flex-shrink-0 transition-all",
                isCorrect
                  ? "border-[#4A9B7F] bg-[#4A9B7F] text-white"
                  : "border-[#DDE8DF] dark:border-[#1E3524] text-[#92A894] hover:border-[#4A9B7F]"
              )}
            ...
            <Input
              ...
              className={cn(
                "flex-1 bg-[#EBF0EC] dark:bg-[#0A180E] border-[#DDE8DF] dark:border-[#1E3524] focus-visible:border-[#4A9B7F] rounded-xl h-10",
                isCorrect && "border-[#4A9B7F] bg-[#D1FAE5] dark:bg-[#064e35]"
              )}
            />
            {isCorrect && (
              <span className="text-[10px] font-bold text-[#4A9B7F] shrink-0">✓ Correct</span>
            )}
          </div>
        );
      })}
        ...
        return (
          <div
            ...
            className={cn(
              ...
              isCorrect
                ? "border-[#4A9B7F] bg-[#D1FAE5] dark:bg-[#064e35] font-bold"
                : "border-[#DDE8DF] dark:border-[#1E3524] bg-[#EBF0EC] dark:bg-[#0A180E]"
            )}
          >
            <span className={cn(
              "w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black shrink-0",
              isCorrect ? "bg-[#4A9B7F] text-white" : "bg-[#DDE8DF] dark:bg-[#1E3524] text-[#92A894]"
            )}>{letter}</span>
            <span className="truncate">{text || <span className="italic text-[#92A894]">Choice {letter}</span>}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="max-w-xl mx-auto fade-in">
      <div className="mb-7">
        ...
        <p className="text-sm text-[#5A7860] dark:text-[#7BAF84] mt-1">Configure the test set before adding questions.</p>
      </div>
      <Card>
        <div>
          <Lbl c="Test Title *" />
            ...
            {(["PRE_TEST", "POST_TEST"] as TestType[]).map((t) => (
              <button key={t} onClick={() => onChange({ ...info, type: t })}
                ...
                <span className="text-[11px] text-[#5A7860] dark:text-[#7BAF84]">
                  {t === "PRE_TEST" ? "Given before the module — establishes baseline" : "Given after the module — measures learning gain"}
                </span>
              </button>
            ))}
          </div>
...
// ─── Step 1 — Questions ───────────────────────────────────────
function StepQuestions({ questions, onChange, onNext, onBack, contexts }: {
  questions: QuestionDraft[]; onChange: (q: QuestionDraft[]) => void;
  onNext: () => void; onBack: () => void; contexts: ContextOption[];
}) {
    ...
    <div className="max-w-5xl mx-auto fade-in">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-nunito text-2xl font-black">Test Questions</h1>
            <p className="text-sm text-[#5A7860] dark:text-[#7BAF84] mt-0.5">
              {questions.length} question{questions.length !== 1 ? "s" : ""} — click to edit
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-4">
        ...
        <div className="bg-white dark:bg-[#132018] border border-[#DDE8DF] dark:border-[#1E3524] rounded-2xl p-3 flex flex-col gap-1 h-fit">
          {questions.map((item, i) => {
            ...
            const isMCItem = item.answerType === "multiple_choice";
            return (
              <button key={i} onClick={() => setActive(i)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    {isMCItem && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#EDE9FE] text-[#8B5CF6]">MC</span>}
          ...
          <Card key={active}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => upd({ ...q, answerType: "number", answer: "", choices: ["", "", "", ""] })}
                  className={cn(
                    "flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-bold border-2 transition-all",
                ...
                <button
                  onClick={() => upd({ ...q, answerType: "multiple_choice", answer: "" })}
                  className={cn(
                    "flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-bold border-2 transition-all",
            ...
            <div>
              <Lbl c="Question Text *" />
              <TA v={q.questionText} onChange={(v) => upd({ ...q, questionText: v })}
                ph="Type the question exactly as students will see it…" rows={4} />
            </div>

            {/* Answer section — MC or open */}
            {isMC ? (
              ...
              {isMC ? (
                ...
                <div className="mt-1 h-8 bg-white dark:bg-[#132018] rounded-lg border border-[#DDE8DF] dark:border-[#1E3524] flex items-center px-3">
                  <span className="text-[11px] text-[#92A894]">
                    Student answer field ({OPEN_ANSWER_TYPES.find((t) => t.value === q.answerType)?.label ?? q.answerType})
                  </span>
                </div>
              )}
            </div>
      ...
      <div className="flex items-center justify-between mt-6">
        ...
        <button onClick={onNext} disabled={!allOk}
          className="px-7 py-3 rounded-xl font-nunito font-extrabold text-sm bg-[#4A9B7F] hover:bg-[#2E7A60] disabled:opacity-40 text-white shadow-[0_4px_14px_rgba(74,155,127,0.3)] transition-all">
          Next → Review
        </button>
      </div>
...
// ─── Step 2 — Review & Save ───────────────────────────────────
function StepReview({ info, questions, onBack, onSave, saving }: {
  info: TestInfo; questions: QuestionDraft[];
  ...
  return (
    <div className="max-w-3xl mx-auto fade-in">
      <div className="mb-7">
        ...
        <h1 className="font-nunito text-2xl font-black">Review & Save</h1>
        <p className="text-sm text-[#5A7860] dark:text-[#7BAF84] mt-1">Confirm the details before creating the test set.</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#132018] border border-[#DDE8DF] dark:border-[#1E3524] rounded-2xl overflow-hidden mb-4">
        ...
        <div className="px-6 py-4 flex items-center gap-4 text-[12px] border-b border-[#DDE8DF] dark:border-[#1E3524] flex-wrap">
          ...
          <span className="text-[#5A7860] dark:text-[#7BAF84]">|</span>
          {(["easy", "average", "difficult"] as Difficulty[]).map((d) => {
            const meta = DIFFICULTY_META[d];
            return diffCounts[d] > 0 ? (
              <span key={d} className="font-bold px-2 py-0.5 rounded-full text-[11px]" style={{ background: meta.bg, color: meta.color }}>
                {meta.label}: {diffCounts[d]}
              </span>
            ) : null;
          })}
        </div>
            ...
            {questions.map((q, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-3 px-4 py-3 bg-[#EBF0EC] dark:bg-[#0A180E] rounded-xl">
                  <div className="w-6 h-6 rounded-full bg-[#4A9B7F] text-white flex items-center justify-center text-[11px] font-black flex-shrink-0 mt-0.5">{q.questionNum}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    {isMCQ ? (
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        {q.context && <span className="text-[10px] text-[#92A894]">{q.context}</span>}
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: diff.bg, color: diff.color }}>{diff.label}</span>
                        <span className="text-[10px] text-[#92A894]">Ans: <strong>{q.answer}</strong></span>
                        {q.melc && <span className="text-[10px] text-[#5A7860] dark:text-[#7BAF84]">{q.melc}</span>}
                      </div>
                    )}
                  </div>
...
export default function CreateTestPage() {
  ...
  const initType     = (searchParams.get("type") ?? "PRE_TEST") as TestType;
  const initModuleId = searchParams.get("moduleId") ?? "";
  ...
  const [info, setInfo] = useState<TestInfo>({ title: "", type: initType, description: "", timeLimit: "", moduleId: initModuleId });
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/teacher/tests/create", {
      ...
      <div className="min-h-screen bg-[#F0F4F1] dark:bg-[#0A180E] text-[#1E293B] dark:text-[#E8F5EC]">
        <div className="sticky top-0 z-10 bg-white dark:bg-[#132018] border-b border-[#DDE8DF] dark:border-[#1E3524] px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-nunito font-extrabold text-[15px]">Create Test Set</span>
          </div>
          <div className="flex items-center gap-1">
            {(["info", "questions", "review"] as Step[]).map((s, i) => (
        ...
        <div className="p-8">
          ...
          {step === "questions" && <StepQuestions questions={questions} onChange={setQuestions} onBack={() => setStep("info")} onNext={() => setStep("review")} contexts={contexts} />}
          {step === "review"    && <StepReview info={info} questions={questions} onBack={() => setStep("questions")} onSave={handleSave} saving={saving} />}
        </div>
      </div>
    </>
  );
